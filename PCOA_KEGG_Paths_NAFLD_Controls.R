...
##Load data
load("Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/R_Datasets/Updated_NAFLD_O_L_KEGGPaths_CPM_Clinical_Data.RData")
...
PWA_BC_treat_NMDS_2 <- adonis2(B_Dist ~  Age + Gender + BMI + HOMA_IR + SBP  + Groups, 
                               data = full, permutations = 999,by = "terms")
...
pcoa_bc_beta_div <-ggplot(ggplot_df, aes(x=PCoA_1, y=PCoA_2,colour = Groups, label = Groups)) + 
  ...
  annotate("text", x = -0.0375, y = 0.025, label = paste(("R² = 0.006")),size=8) +
  labs(x = "NMDS 1",
       y = "NMDS 2")

pdf("Documents/Phd/Metabolic_Diseases/Submissions/Cell/Supplementary Figures/S_Figure_2_Kegg.pdf", width = 15, height = 10)
pcoa_bc_beta_div
dev.off()
