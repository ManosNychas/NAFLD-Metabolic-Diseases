...
load("Users/manosnychas/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/R_Datasets/Updated_NAFLD_O_L_Metaphlan3_Clinical_Data.RData")
...
TREE <- read_tree("Users/manosnychas/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/Metaphlan_3_parsed_species_only.nwk")
...
beta_distances_dataset <- function(dataset){
  ...
  new_full = cbind(dataset,current_metadata)
  
  physeq <- metaphlanToPhyloseq(t(dataset))
  SAM <-  sample_data(data.frame(current_metadata))
  physeq <- merge_phyloseq(physeq, SAM, TREE)
  
  
  #Unifracs
  ord_w_unifrac_nmds <- 
    ordinate(physeq, method="NMDS", distance="unifrac", weighted=TRUE) %>% 
    vegan::scores(.)%>% 
    as.data.frame() %>% 
    dplyr::select(1,2)
  ...
  distance_list <- list(
    WUnifrac_NMDS = ord_w_unifrac_nmds
  )
...
distance_dataset <- Distance_Full$distances$WUnifrac_NMDS
