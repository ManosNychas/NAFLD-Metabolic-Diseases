...
## Significnatly Different Metabolites for Comorbidities
Commorbidities_Metabolites <- 
   read.csv("~/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/Metabolomics/direction_All_Comp_significant_metabolites_csv.csv") %>% 
   mutate(Direction = ifelse(Direction == "up",1,0)) %>% 
...
Commorbidities_Metabolites_1 <- 
  read.csv("~/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/Metabolomics/OLR_to_Lasso_significnat_metabolites.csv",sep = "\t") %>% 
...
Metabolites_NAFLD_L <-
   ...
   column_to_rownames(var = "X") %>% 
   t() %>%  
   .[which(rownames(.) %in% rownames(Species_NAFLD_L)),] %>% 
   as.data.frame() %>% 
   .[order(rownames(.)),] 
...
Metabolites_NAFLD_All <-
   ...
   column_to_rownames(var = "X") %>% 
   t() %>%  
   .[which(rownames(.) %in% rownames(Species_NAFLD_All)),] %>% 
   as.data.frame() %>% 
   .[order(rownames(.)),] 
...
Name_Change_metabolites <-
  read.csv("~/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/Metabolomics/id_to_name.csv") %>% 
...
write_rds(lists,"~/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/Figures_2/Figure 4/Mediation/Metab_Cors.rds")
...
## Renaiming metabolites to resolve issues when plotting, these are later changed manually in the plot in innkscape 
DF <-  
   DF_FLI %>% 
left_join(DF_GGT) %>% 
   left_join(DF_ALT) %>% 
...
## Circos Plot
pdf("~/Documents/Phd/HKI_PC/Metabolic_Diseases/Updated_Human3/NAFLD_O_L/Figures_2/Figure 4/Chor_plot.pdf", width = 15, height = 15)
